# [std::string::String](https://doc.rust-lang.org/std/string/struct.String.html) and [std::str](https://doc.rust-lang.org/std/primitive.str.html)
...
let s = String::new();
```
This creates a new empty `String`. Given that the `String` is empty, this will not allocate any initial buffer. While that means that this initial operation is very inexpensive, it may cause excessive allocation later when you add data. If you have an idea of how much data the `String` will hold, consider the `with_capacity` method to prevent excessive re-allocation.

`&str` is a string slice (`&[u8]`) that always points to a valid UTF-8 sequence, and can be used to view into a `String`. It is also the type of string literals, `&'static str`. A &str is made up of two components: a pointer to some bytes, and a length.
